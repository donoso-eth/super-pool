...
describe('Super Pool Global', function () {
  beforeEach(async () => {
    ...
    console.log(superPoolTokenAddress)
    ...
    const operation = sf.cfaV1.createFlow({
      receiver: superPoolTokenAddress,flowRate:"5",superToken:TOKEN1
    })

    await operation.exec(user1)
    await printPeriod(superTokenPool);
    let erc777 = await ERC777__factory.connect(TOKEN1, deployer);
    await waitForTx(erc777.send(superPoolTokenAddress, 10, '0x'));

    await waitForTx(superTokenPool.mockYield(1))

  });

  it('Should Start with No Fake values created', async function () {
  
   let period = await printPeriod(superTokenPool);
   await  printUser(superTokenPool,deployer.address)
